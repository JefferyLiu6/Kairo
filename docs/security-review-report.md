...
| Shared CSRF/rate-limit storage    | Current CSRF tokens and rate-limit buckets are process-local. This is fine for a single Render process but weak for multi-worker deployments. | Use Redis, SQLite, or another shared store before scaling beyond one backend process.                   |
